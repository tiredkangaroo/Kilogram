...
class Routes {
    ...
    async post(req, res) {
        ...
        //the scope
        if (!postID) {
            return res.status(400).send("The ID is a required parameter.");
        }
        try {
    ...
    async delete(req, res) {
        const postID = req.body.postID.toString("hexc");
        if (!postID) {
            return res.status(400).send("PostID is required.");
        }
        const post = await Post.findOne({ _id: new ObjectId(postID) });
        if (!post) {
            res.status(404).send("Post deletion process was unsucessful. Post was not found.");
        }
        if (post.imageKey) {
            unlink(`storage/${post.imageKey}`, () => { });
        }
        ...
        if (deletePost) {
            ...
            return res.status(404).send("Post deletion process was unsucessful.");
        }
    }
    async heart(req, res) {
        ...
        if (Object.keys(post.likerIDs).includes(req.user._id.toString())) {
            const userID = req.user._id.toString("hex");
            const the_current_date = new Date();
            const modifiedLikerIDs = { ...post.likerIDs, [userID]: [the_current_date] };
